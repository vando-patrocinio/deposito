...
export default function AIPreventivePanel({ onClose }) {
    ...
    <div onClick={onClose}
      ...
      <div onClick={(e) => e.stopPropagation()} data-testid="ai-preventive-panel"
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
          <div>
            <h2 style={{ margin: 0, fontSize: 22, fontWeight: 800, color: "#0f172a" }}>🤖 IA Preventivas</h2>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                <thead>
                  <tr>
                    ...
                    <th style={css.th}>30d</th>
                    <th style={css.th}>Dias ativos</th>
                    <th style={css.th}>Ritmo/dia</th>
                    <th style={css.th}>Carga hoje</th>
                    <th style={css.th}>Vagas livres</th>
                  </tr>
                </thead>
                <tbody>
          ...
          <Card title={`📋 ${suggestions.length} sugestão(ões) pendente(s)`} data-testid="ai-suggestions-card">
            {suggestions.length === 0 ? (
              ...
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                <thead>
                  <tr>
                    <th style={css.th}>Cliente · OLT</th>
                    <th style={css.th}>Sinal</th>
                    <th style={css.th}>Técnico sugerido</th>
                    <th style={css.th}>Cap. téc.</th>
                    <th style={css.th}>Ações</th>
                  </tr>
                </thead>
                <tbody>
                  {suggestions.map((s) => (
                    <tr key={s.id} data-testid={`ai-sugg-row-${s.id}`}>
                      ...
                      <td style={css.td}>
                        <button style={{ ...css.btnGhost, background: "#dcfce7", color: "#15803d", borderColor: "#86efac" }}
                                onClick={() => accept(s.id)} data-testid={`ai-accept-${s.id}`}>
                          ✓ Aceitar
                        </button>
                        <button style={{ ...css.btnGhost, background: "#fee2e2", color: "#991b1b", borderColor: "#fca5a5" }}
                                onClick={() => reject(s.id)} data-testid={`ai-reject-${s.id}`}>
                          ✕ Recusar
                        </button>
                      </td>
                    </tr>

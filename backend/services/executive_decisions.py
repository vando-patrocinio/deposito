...
async def create_decision(cid: str, payload: dict) -> dict:
    decision_text = (payload.get("decision") or "").strip()
    ...
    doc = {
        ...
        "related_kpi": payload.get("related_kpi"),
        "priority": priority,
...
async def update_status(cid: str, decision_id: str, payload: dict) -> dict:
    ...
    if new_status:
        ...
        if new_status == "approved" and payload.get("approved_by"):
            set_doc["approved_by"] = payload.get("approved_by")
    for k in ("owner", "deadline", "context", "priority"):

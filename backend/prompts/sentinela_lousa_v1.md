# Sentinela Lousa — Prompt Canônico V1

> **Fonte de verdade do prompt.** Versionado no GitHub.
> Cada `git push` desta pasta sobrescreve o `system_prompt` no
> `aihub_agents.Sentinela Lousa` na próxima boot do backend
> ou via endpoint `POST /api/aihub/prompts/Sentinela Lousa/reload-prompt`.
>

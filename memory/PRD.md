...
✅ **WhatsApp por QR Code (Baileys)** (10/05/2026): Node.js sidecar `@whiskeysockets/baileys` (porta 3002, supervisor) + FastAPI proxy `/api/whatsapp-baileys/{qr,status,send,logout,inbound,messages}` + React `WhatsAppQRPanel` com QR PNG renderizado, polling adaptativo (3s desconectado, 8s conectado). Sessão persiste em `/app/whatsapp-service/auth_info/`. Webhook inbound auto-linka telefone ao Subscriber e salva em `aihub_wa_messages`. Testing agent: 7/7 backend + frontend 100%.
